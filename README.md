...
- **通道**（9 种）：通用 Webhook（带 HMAC 签名）、Bark、Server酱³、Telegram、企业微信、钉钉、飞书、短信回发（可选卡）、**邮箱 SMTP**
...
把短信以邮件形式发出去。相比 Webhook，**不要求手机和接收端在同一网络**——
手机在蜂窝网下也能把验证码送到，适合「手机收码 → PC 脚本自动取码」这类场景。

「通道」页新增时选 **邮箱 SMTP**，四个字段这样填：

| 字段 | 含义 | 示例 |
|------|------|------|
| SMTP 服务器 | `host:port`，端口 **465** 走隐式 SSL，**587 / 25** 走 STARTTLS | `smtp.126.com:465` |
| 邮箱账号（发件人） | SMTP 登录账号，同时作为 `From` | `abc@126.com` |
| 密码 / 授权码 | **授权码**，不是邮箱登录密码 | `xxxxxx` |
| 收件人 | 多个用英文逗号分隔 | `abc@126.com` |

- 发件主题固定为 `【短信转发】<发送号码>`，正文即短信原文（UTF-8）。
- 126 / 163 / QQ 邮箱必须用 **授权码** 登录，用登录密码会被拒；各家 SMTP / IMAP 地址见服务商说明。
- 自动化取码：用任意 IMAP 客户端轮询收件箱、按主题 `【短信转发】` 过滤、正则取 4~8 位数字即可。
  建议同时用邮件的 `INTERNALDATE`（服务器接收时间）做时间门槛，避免取到历史旧码——
  IMAP 的 `SINCE` 只精确到「天」，单独用它不足以防旧码。
- 发信在 `Dispatchers.IO` 上执行，失败会记入「日志」页并按 6 次指数退避重试，
  授权码错误、服务器连不上等原因都能在日志里直接看到。
